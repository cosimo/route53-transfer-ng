...
def get_zone(r53_client, zone_name, vpc):
    ...
    for zone in hosted_zones:
        ...
        if (is_private and list_private_zones) or (
            not is_private and not list_private_zones
        ):
    ...
    for zone in matching_zones:
        ...
        # TODO validate
        z_vpc_id = (
            z.get("HostedZone", {}).get("VPCs", {}).get("VPC", {}).get("VPCId", "")
        )
...
def load(r53, zone_name, file_in, **kwargs):
    ...
    changes = compute_changes(
        zone, existing_records, desired_records, use_upsert=use_upsert
    )
...
def compute_changes(zone, existing_records, desired_records, use_upsert=False):
    ...
    if to_add or to_delete:
        for record in sort_by_name(to_add):
            op_type = (
                "UPSERT" if use_upsert and is_in_set(record, to_delete) else "CREATE"
            )
...
def dump(r53_client, zone_name, output_file, **kwargs):
    ...
    if not zone:
        exit_with_error(
            "ERROR: {} zone {} not found!".format(
                "Private" if vpc.get("is_private") else "Public", zone_name
            )
        )
...
def run(params):
    ...
    if params.get("dump"):
        ...
        dump(r53_client, zone_name, get_file(filename, "w"), format=format, vpc=vpc)
